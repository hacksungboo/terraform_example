# test10_alb_route53/loadbalancer.tf
...
resource "aws_lb_listener" "https" {
  ...
  # route53.tf 에서 만든 인증서의 arn (직접 발급받아서 사용할 경우)
  #certificate_arn   = aws_acm_certificate_validation.cert.certificate_arn
  
  # 여기에서는 route53.tf 를 실행하지 않고 미리 발급받은 인증서의 arn을 사용한다.
...
# ALB 로 부터 요청을 전달받을 모든 ec2를 대상그룹에 등록한다.
resource "aws_lb_target_group_attachment" "web_attach" {
  # 변수에 정의된 count 숫자만큼 연결한다
  count = var.ec2_count
