...
# 3. 우리가 직접 만든 최신 ami를 가져오는 필터 설정
data "aws_ami" "latest_web" {
  most_recent = true
  # 우리가 직접 만든 AMI 의 owner 는 "self" 이다
  owners      = ["self"]
  filter {
    name   = "name"
    # packer 에서 만든 이름을 참조할수 있도록 한다
    # ami_name        = "ami-web-{{timestamp}}"
    # packer 에서 위와 같은 ami 이름을 부여했기 때문에 아래처럼 작성한다
    values = ["ami-web-*"]
  }
}
...
resource "aws_launch_template" "lt" {
  ...
  # ami 혹은 user_data 가 변경이 되었을때 default 버전을 번경하도록 한다
...
resource "aws_autoscaling_group" "asg" {
  ...
  launch_template {
    ...
    # launch template 의 소스코드가 바뀌어서 버전번호가 올라가면 바로 감지할 수 있도록 한다
