...
resource "aws_subnet" "public_subnet" {
    ...
    # data.aws_availability_zones.available.names는 배열인데, 거기에는 여러개의 가용영역 데이터가 들어있다.
    # 그 중에서 0번 방에 있는 데이터를 연결한다.
    availability_zone = data.aws_availability_zones.available.names[0]

    map_public_ip_on_launch = true # 이 방에 생기는 서버는 무조건 공인 ip를 받는다.
    tags = {
         Name = "lecture-subnet" 
    }
}
...
resource "aws_instance" "my_ec2" {
    ...
    vpc_security_group_ids  = [aws_security_group.ssh_sg.id]   # 보안그룹 (여러개 등록 가능)
...
output "instance_public_ip" {
    description = "만들어지 ec2 의 public ipv4 주소"
...
resource "local_file" "ansible_config" {
    ...
    # inventory 파일의 경로와 ssh 보ㅓ안 확인 (host key checking) 을 자동으로 설정
    content = <<-EOF
        [defaults]
...
resource "terraform_data" "wait_for_instance" {
    ...
    # ec2 인스턴스의 id 가 변경되었다면 다시 실행하도록 방아쇠를 설치한다
    # 즉, ec2 가 새롭게 만들어지면 이 블럭이 다시 실행되고 결과적으로 sleep 30이 다시 실행된다.
...
resource "terraform_data" "ansible_run" {
    ...
    # ec2 가 새로 만들어지면 ansible playbook을 실행하도록 여기도 방아쇠를 걸어 놓는다.
    triggers_replace = {
        instance_id = aws_instance.my_ec2.id
        always_run = "${timestamp()}" # 실행할때 마다 시간이 달리지므로 무조건 플레이북이 실행된다.
    }
