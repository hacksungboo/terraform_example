...
resource "aws_route_table" "public_rt" {
    vpc_id = aws_vpc.main.id
    # vpc 대역이 아닌 다른 모든 ip 에 대한 요청은 internet gateway 로 보낸다
    route {
...
resource  "aws_route_table_association" "public_a" {
    ...
    # 여기 정의된 routing table로 연결하겠다는 의미
...
resource "aws_instance" "nat_ec2" {
    ...
    # 다른 ec2에서 받은 트레픽ㅇ르 외부로 전달해야 하기 때문에
...
# ----------------------------------------------------
# Public Subnet에서 172.16.8.0/24로 갈 때 대장 EC2를 거치도록 설정 (my_ec2는 아래에 정의할 예정)
# aws_route 는 미리 만들어놓은 aws_route_table 에 라우팅 정보를 추가하는 용도로 사용할 수 있다.
...
resource "aws_instance" "my_ec2" {
    ...
    # 대장 ec2 가 만들어지는 시점에 tailscale 에 접속해서 vpn 연결을 활성화 하는 작업을 script 구현하겠다
...
data "tailscale_device" "my_ec2_device" {
  ...
  wait_for   = "180s" # 약간의 시간이 걸릴수 있으니 기기가 리스트에 뜰 때까지 테라폼이 기다려줍니다.
...
resource "terraform_data" "wait_for_instance"{
    ...
    provisioner "local-exec" {
        # user_data에서 Tailscale 세팅이 완벽히 끝날 때까지 대기 (180초)
